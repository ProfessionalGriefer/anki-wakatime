...
import globals as g
from __init__ import ankiConfig, ApiDialogWidget
# Custom imports
...
def handle_activity(card: Card, is_write=False):
    ...
    # Gets the frontside of the card without the HTML
    entity: str = next(iter(card.next()))
    ...
    if entity != last_file or enough_time_passed(timestamp, is_write):
        collection = card.col()
        deck_id = card.did()
        project = collection.decks.name(deck_id)

        # folders = None  # Don't know what to do with that
